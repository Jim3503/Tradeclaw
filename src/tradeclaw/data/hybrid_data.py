...
def get_stock_data_hybrid(code: str, days: int = 30) -> dict:
    ...
    # 获取历史数据（baostock），如果失败则使用AkShare
    ...
    try:
        ...
        if lg.error_code == '0':
            ...
            print(f"BaoStock登录失败，尝试使用AkShare获取历史数据...")
            raise Exception(f"BaoStock login failed: {lg.error_msg}")
    except Exception as e:
        print(f"BaoStock获取{code}历史数据失败: {e}")
        print(f"尝试使用AkShare作为备用数据源...")

        # 回退到AkShare
        try:
            from tradeclaw.data.akshare_data import get_stock_kline_akshare
            kline_data = get_stock_kline_akshare(code, days=days)

            if kline_data:
                print(f"✅ AkShare成功获取{len(kline_data)}条历史数据")
            else:
                print(f"⚠️ AkShare也未获取到数据")
        except Exception as akshare_error:
            print(f"AkShare获取数据也失败: {akshare_error}")
            # 如果两种方式都失败，返回空列表
            kline_data = []
